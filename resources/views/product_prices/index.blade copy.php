                ...
                <tbody>
                    ...
                    <tr role="row" class="{{ ($key%2==0)?'odd':'even' }}" id="{{ $value->id }}">
                        <td style="width:215px !important">
                            ...
                            <div class="editInput" style="display: none; width:215px !important">
                                <select name="product_id" class="form-control show-tick product_id varInput"
                                    ...
                                    <option value="{{ $getDataProduct->id }}"
                                        data-subtext="{{ $getDataProduct->distributor_name }}"
                                        {{$getDataProduct->product_id == $value->product_id  ? 'selected' : ''}}>
                                        {{ $getDataProduct->product_name }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <td style="width:50px !important">
                            ...
                            <input class="editInput total_product form-control input-sm varInput" type="text"
                                name="total_product" value="{{ $value->total_product }}"
                        ...
                        <td style="width:77px !important">
                            <span
                                class="editSpan purchase_price">{{ Helpers::MoneyFormat($value->purchase_price) }}</span>
                            <input id="purchase_price_{{ $value->id }}"
                                class="editInput purchase_price form-control input-sm" type="text"
                                value="{{ number_format($value->purchase_price, 0, ',', '.') }}"
                                style="display: none; width:77px" onkeyup="priceRow(this)">
                            <input id="purchase_price_{{ $value->id }}_ori" type="hidden"
                                class="form-control text-center varInput" name="purchase_price"
                                value="{{ $value->purchase_price }}">
                        </td>
                        <td style="width:77px !important">
                            <span
                                class="editSpan selling_price">{{ Helpers::MoneyFormat($value->selling_price) }}</span>
                            <input id="selling_price_{{ $value->id }}"
                                class="editInput selling_price form-control input-sm" type="text"
                                value="{{ number_format($value->selling_price, 0, ',', '.') }}"
                                style="display: none; width:77px" onkeyup="priceRow(this)">
                            <input id="selling_price_{{ $value->id }}_ori" type="hidden"
                                class="form-control text-center varInput" name="selling_price"
                        ...
                        <td style="width:77px !important">
                            <span
                                class="editSpan delivery_date">{{ Helpers::customDate($value->delivery_date, 'short') }}</span>
                            <input
                                class="editInput delivery_date form-control input-sm varInput mask_date date_max_today"
                                type="text" name="delivery_date"
                                value="{{ date('d/m/Y', strtotime($value->delivery_date)) }}"
                                style="display: none; width:77px">
                        </td>
                        <td class="text-center" style="width:100px !important">
                            ...
                            <div class="btn-group btn-group-sm save-confirm-cancel">
                                <button type="button" class="btn btn-success waves-effect saveBtn"
                                    style="float: none; display: none;" onclick="saveButton(this)">
                                    <i class="material-icons">save</i>
                                </button>
